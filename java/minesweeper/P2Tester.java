...
public class P2Tester {
    ...
    private void check_Minefield_7() {
        //        7. logic test (for honors section): call the constructor, then call mineLaying(int, String), and then count the number of MineCell objects per row to assert the number is correct
        assertClassFound("Minefield");
        assertClassFound("MineCell");
        assertClassFound("InfoCell");
        if (isMethodExisting("Minefield", "mineLaying", int.class, String.class)) {
            final int row = 5;
            final int col = 7;
            final int mines = 15;
            Minefield mf = new Minefield(row, col, mines);

            // reset minefield and redo mineLaying
            for (int i = 0; i < row; i++)
                for (int j = 0; j < col; j++)
                    mf.setMineCell(i, j, null);
            mf.mineLaying(mines, "uniform");
            ...
            for (int i = 0; i < row; i++)
            {
                count = 0;
                for (int j = 0; j < col; j++)
                    if (mf.getCellByRowCol(i, j) instanceof MineCell)
                        count++;
                assertEquals("Number of mines in row " + i + " is incorrect.", mines/row, count);
            }
        }
    }
